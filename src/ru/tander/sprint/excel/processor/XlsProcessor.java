...
import ru.tander.sprint.excel.listener.FTHSSFListener;
...
public class XlsProcessor extends AbortableHSSFListener implements IProcessor  {
	...
	private void appendCellAfter(int columnIndex, int rowIndex,  String data) throws XMLStreamException, XlsParserException{
		...
		if(prevColumnIndex > columnIndex && rowIndex == this.prevRowIndex){
			throw new XlsParserException("Нарушен порядок разбора ячеек" + "col: "+this.prevColumnIndex + " " + columnIndex + ", row: "+ this.prevRowIndex +" " + rowIndex);
		}
		if( rowIndex < this.prevRowIndex){
			throw new XlsParserException("Нарушен порядок разбора строк");
		}
	...
	private void initTransformation() throws Exception{
		...
		if(this.inputStream == null){
			...
			if(this.inputStream == null){
				throw new NullPointerException("Входной поток необходимо указать xlsParserWriter.setInputStream(...)");
			}
		}
...
//        Assert.assertNotNull(req);
	...
	private void finishTransformation() throws Exception{
		...
		this.inputStream = null; //второй раз уже не запустится. Нужно заново устанавливать inputStream
